...
// guardar
$(".fmr_cajas").submit(function(event) {
    ...
    // Supongamos que este código se ejecuta después de que se ha guardado con éxito una nueva caja
    var nuevacaja = {
        codigo: $("#codigo").val(),
        nombre: $("#nombre").val(),
        resumen: $("#resumen").val()
    ...
    if (codigo == "" || nombre == "" || resumen == "") {
        ...
        // Hacer la solicitud AJAX para guardar la nueva caja
        $.ajax({
            type: 'POST',
            url: 'ajax/cajasajax.php',
            data: {
...
function editar(id) {
    ...
    var urlprocess = 'ajax/cajasajax.php';
    $.ajax({
        type: 'POST',
        url: urlprocess,
...
$(".fmr_cajas_editar").submit(function(event) {
    event.preventDefault();
...
function eliminar(id) {
    ...
    Swal.fire({
        title: '¿Estás seguro?',
        text: '¿Seguro que deseas eliminar el caja?',
        icon: 'warning',
