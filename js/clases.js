...
function cargar_tabla() {
    ...
    var urlprocess = 'ajax/clasesajax.php';
    $.ajax({
        type: 'POST',
        url: urlprocess,
        data: 'proceso=get',
        ...
        success: function(data) {
            ...
            if (data.length > 0) {
                ...
                var options = {
                    valueNames: ['codigo', 'nombre', 'resum'],
                ...
                var searchList = new List('myTable', { valueNames: ['codigo', 'nombre', 'resum'], page: 5 });
                input.addEventListener('input', function() {
                    searchList.search(input.value);
                });
            } else {
                // Mostrar un mensaje indicando que no hay clases disponibles
                $('#myTable tbody').html('<tr><td colspan="4" class="text-center">No hay clases disponibles</td></tr>');
            }
        },
        error: function() {
...
function eliminar(id) {
    ...
    Swal.fire({
        title: '¿Estás seguro?',
        text: '¿Seguro que deseas eliminar el clase?',
        icon: 'warning',
...
function generar() {
    ...
    window.open('pdfs/generar_pdf_clases.php', '_blank');
