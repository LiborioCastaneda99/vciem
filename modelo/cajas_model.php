<?php
require_once('../modelo/Conexion.php');  // Se carga la caja conexion

class cajasModel extends Conexion
{
    ...
    public static function get()
    {
        ...
        try {
            ...
            $sql = "SELECT id, codigo, nombre, resum FROM tbcajas";
    ...
    public static function get_id($id)
    {
        ...
        try {
            $query = "SELECT `id`, `codigo`, `nombre`, `resum` FROM tbcajas WHERE id = $id";
            ...
            if ($rows) {
                // Devolver el array JSON con todos los tbcajas
    ...
    public static function guardar($datos)
    {
        ...
        try {
            ...
            $query = "SELECT COUNT(*) as count FROM tbcajas WHERE codigo = :codigo";
            $stmt = $dbconec->prepare($query);
            ...
            if ($result['count'] > 0) {
                ...
                $query = "INSERT INTO tbcajas (codigo, nombre, resum) VALUES (:codigo, :nombre, :resumen)";
                $stmt = $dbconec->prepare($query);
                $stmt->bindParam(':codigo', $codigo);
                ...
                if ($stmt->execute()) {
                    ...
                    $response = array('status' => 'success', 'message' => 'La caja se ha guardado correctamente');
                } else {
                    // Si hubo un error en la inserción, devuelve un mensaje de error
                    $response = array('status' => 'error', 'message' => 'Error al guardar la caja');
                }
    ...
    public static function modificar($datos)
    {
        ...
        try {
            ...
            $id = $datos['id'];
            // Consulta para verificar la existencia del código
            $query = "SELECT COUNT(*) as count, codigo FROM tbcajas WHERE id = :id";
            $stmt = $dbconec->prepare($query);
            ...
            if ($codigo != $codigo_bd) {
                $queryC = "SELECT COUNT(*) as count, codigo FROM tbcajas WHERE codigo = :codigo";
                ...
                if ($result['count'] > 0) {
                    ...
                    // Realiza la inserción en la base de datos (ajusta esto según tu configuración)
                    $query = "UPDATE tbcajas SET codigo=:codigo, nombre=:nombre, resum=:resumen WHERE id=:id";
                    $stmt = $dbconec->prepare($query);
                    $stmt->bindParam(':id', $id);
                    $stmt->bindParam(':codigo', $codigo);
                    $stmt->bindParam(':nombre', $nombre);
                    ...
                    if ($stmt->execute()) {
                        // Si la inserción fue exitosa, devuelve un mensaje o los datos actualizados
                        $response = array('status' => 'success', 'message' => 'La caja se ha modificado exitosamente.');
                    } else {
                        // Si hubo un error en la inserción, devuelve un mensaje de error
                        $response = array('status' => 'error', 'message' => 'Error al modificar la caja.');
                    }
                ...
                // Realiza la inserción en la base de datos (ajusta esto según tu configuración)
                $queryUdt = "UPDATE tbcajas SET codigo=:codigo, nombre=:nombre, resum=:resumen WHERE id=:id";
    ...
    public static function eliminar($id)
    {
        ...
        try {
            $activo = 0;
            // Realiza la inserción en la base de datos (ajusta esto según tu configuración)
            $query = "UPDATE tbcajas SET activo=:activo WHERE id=:id";
            $stmt = $dbconec->prepare($query);
            $stmt->bindParam(':id', $id);
            ...
            if ($stmt->execute()) {
                // Si la inserción fue exitosa, devuelve un mensaje o los datos actualizados
                $response = array('status' => 'success', 'message' => 'caja eliminada exitosamente.');
            } else {
                // Si hubo un error en la inserción, devuelve un mensaje de error
                $response = array('status' => 'error', 'message' => 'Error al eliminar la caja.');
            }
    ...
    public static function combo_caja($searchTerm, $id)
    {
        ...
        try {
            ...
            if ($id != '') {
                ...
                // Mostrar resultados
                $sql = "SELECT id, codigo, nombre FROM tbcajas where id=:id";
                ...
                if ($searchTerm == '') {
                    ...
                    $sql = "SELECT id, codigo, nombre FROM tbcajas ORDER BY nombre LIMIT :limit";
                    $stmt = $dbconec->prepare($sql);
                    ...
                    // Mostrar resultados
                    $sql = "SELECT id, codigo, nombre FROM tbcajas WHERE nombre like :nombre or codigo like :nombre ORDER BY nombre LIMIT :limit";
                    $stmt = $dbconec->prepare($sql);
