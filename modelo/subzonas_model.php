...
class subzonasModel extends Conexion
{
    ...
    public static function get_id($id)
    {
        ...
        try {
            $query = "SELECT `id`, `codigo`, `zona`, `nombre`, `resum` FROM tbsubzonas WHERE id = $id";
            $stmt = $dbconec->prepare($query);
            $stmt->execute();
    ...
    public static function guardar($datos)
    {
        ...
        try {
            ...
            $query = "SELECT COUNT(*) as count FROM tbsubzonas WHERE codigo = :codigo";
            $stmt = $dbconec->prepare($query);
            ...
            if ($result['count'] > 0) {
                ...
                $query = "INSERT INTO tbsubzonas (codigo, zona, nombre, resum) VALUES (:codigo, :subzona, :nombre, :resumen)";
                $stmt = $dbconec->prepare($query);
                $stmt->bindParam(':codigo', $codigo);
                ...
                if ($stmt->execute()) {
                    ...
                    $response = array('status' => 'success', 'message' => 'El subzona se ha guardado correctamente');
                } else {
                    // Si hubo un error en la inserción, devuelve un mensaje de error
                    $response = array('status' => 'error', 'message' => 'Error al guardar el subzona');
                }
    ...
    public static function modificar($datos)
    {
        ...
        try {
            ...
            $id = $datos['id'];
            // Consulta para verificar la existencia del código
            $query = "SELECT COUNT(*) as count, codigo FROM tbsubzonas WHERE id = :id";
            $stmt = $dbconec->prepare($query);
            ...
            if ($codigo != $codigo_bd) {
                $queryC = "SELECT COUNT(*) as count, codigo FROM tbsubzonas WHERE codigo = :codigo";
                ...
                if ($result['count'] > 0) {
                    ...
                    // Realiza la inserción en la base de datos (ajusta esto según tu configuración)
                    $query = "UPDATE tbsubzonas SET codigo=:codigo, zona=:subzona, nombre=:nombre, resum=:resumen WHERE id=:id";
                    $stmt = $dbconec->prepare($query);
                    $stmt->bindParam(':id', $id);
                    $stmt->bindParam(':codigo', $codigo);
                    $stmt->bindParam(':subzona', $subzona);
