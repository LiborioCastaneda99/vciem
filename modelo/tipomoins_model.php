...
class tipomoinsModel extends Conexion
{
    ...
    public static function get()
    {
        ...
        try {
            ...
            $sql = "SELECT COUNT(*) ";
            $sql .= " AS allcount FROM tbtipomoin";
            $sql .= " WHERE activo = 1";
            error_log("sql => ".$sql);
            ...
            $sql = "SELECT id, codigo, nombre, resum FROM tbtipomoin";
    ...
    public static function get_id($id)
    {
        ...
        try {
            $query = "SELECT `id`, `codigo`, `nombre`, `resum` FROM tbtipomoin WHERE id = $id";
            ...
            if ($rows) {
                // Devolver el array JSON con todos los tbtipomoin
                echo json_encode($rows);
            } else {
                $data = "No hay tipomoin";
    ...
    public static function guardar($datos)
    {
        ...
        try {
            ...
            if ($result['count'] > 0) {
                ...
                $query = "INSERT INTO tbtipomoin (codigo, nombre, resum) VALUES (:codigo, :nombre, :resumen)";
                $stmt = $dbconec->prepare($query);
                $stmt->bindParam(':codigo', $codigo);
                ...
                if ($stmt->execute()) {
                    ...
                    $response = array('status' => 'success', 'message' => 'El tipomoin se ha guardado correctamente');
                } else {
    ...
    public static function eliminar($id)
    {
        ...
        try {
            $activo = 0;
            // Realiza la inserción en la base de datos (ajusta esto según tu configuración)
            $query = "UPDATE tbtipomoin SET activo=:activo WHERE id=:id";
            $stmt = $dbconec->prepare($query);
            $stmt->bindParam(':id', $id);
            ...
            if ($stmt->execute()) {
                // Si la inserción fue exitosa, devuelve un mensaje o los datos actualizados
                $response = array('status' => 'success', 'message' => 'tipomoin eliminada exitosamente.');
            } else {
                // Si hubo un error en la inserción, devuelve un mensaje de error
                $response = array('status' => 'error', 'message' => 'Error al eliminar la tipomoin.');
            }
    ...
    public static function combo_tipomoins($searchTerm, $id)
    {
        $dbconec = Conexion::Conectar();
        try {
            $numberofrecords = 5;
            if ($id != '') {
                ...
                // Mostrar resultados
                $sql = "SELECT id, codigo, nombre FROM tbtipomoin where id=:id and activo = 1";
                ...
                if ($searchTerm == '') {
                    ...
                    $sql = "SELECT id, codigo, nombre FROM tbtipomoin where activo = 1 ORDER BY nombre LIMIT :limit";
                    $stmt = $dbconec->prepare($sql);
                    ...
                    // Mostrar resultados
                    $sql = "SELECT id, codigo, nombre FROM tbtipomoin WHERE nombre like :nombre or codigo like :codigo and activo = 1 ORDER BY nombre LIMIT :limit";
                    $stmt = $dbconec->prepare($sql);
    ...
    public static function combo_tipofact($searchTerm, $id)
    {
        $dbconec = Conexion::Conectar();
        try {
            $numberofrecords = 5;
            if ($id != '') {
                ...
                // Mostrar resultados
                $sql = "SELECT id, codigo, nombre FROM tbtipofact where id=:id and activo = 1";
                $stmt = $dbconec->prepare($sql);
                $stmt->bindValue(':id', $search, PDO::PARAM_STR);
                $stmt->execute();
                //Variable en array para ser procesado en el ciclo foreach
                ...
                if ($searchTerm == '') {
                    ...
                    // Obtener registros a tarves de la consulta SQL
                    $sql = "SELECT id, codigo, nombre FROM tbtipofact where activo = 1 ORDER BY nombre LIMIT :limit";
                    $stmt = $dbconec->prepare($sql);
                    $stmt->bindValue(':limit', (int) $numberofrecords, PDO::PARAM_INT);
                    $stmt->execute();
                    $lstResult = $stmt->fetchAll();
                    ...
                    $search = $searchTerm; // Search text
                    // Mostrar resultados
                    $sql = "SELECT id, codigo, nombre FROM tbtipofact WHERE nombre like :nombre or codigo like :codigo and activo = 1 ORDER BY nombre LIMIT :limit";
                    $stmt = $dbconec->prepare($sql);
                    $stmt->bindValue(':codigo', '%' . $search . '%', PDO::PARAM_STR);
                    $stmt->bindValue(':nombre', '%' . $search . '%', PDO::PARAM_STR);
                    $stmt->bindValue(':limit', (int) $numberofrecords, PDO::PARAM_INT);
