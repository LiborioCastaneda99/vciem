...
class transportadoresModel extends Conexion
{
        ...
        try {
            ...
            if ($searchValue != '') {
                $searchQuery = " AND (
					...
					resum LIKE :resum or 
					conduc LIKE :conductor or 
					est LIKE :estado) ";
                $searchArray = array(
                    'codigo' => "%$searchValue%",
                    'nombre' => "%$searchValue%",
                    'resum' => "%$searchValue%",
                    'conductor' => "%$searchValue%",
                    'estado' => "%$searchValue%"
                );
            }
            ...
            $sql = "SELECT COUNT(*) ";
            $sql .= " AS allcount FROM tbtransportadores";
            $sql .= " WHERE activo = 1";
            error_log("sql => " . $sql);
            ...
            $sql = "SELECT id, codigo, nombre, resum, conduc As conductor, est As estado FROM tbtransportadores";
    ...
    public static function get_id($id)
    {
        ...
        try {
            $query = "SELECT `id`, `codigo`, `nombre`, `resum`, `conduc`, `est` FROM tbtransportadores WHERE id = $id";
            ...
            if ($rows) {
                // Devolver el array JSON con todos los tbtransportadores
                echo json_encode($rows);
            } else {
                $data = "No hay transportadores";
    ...
    public static function guardar($datos)
    {
        ...
        try {
            ...
            $query = "SELECT COUNT(*) as count FROM tbtransportadores WHERE codigo = :codigo";
            $stmt = $dbconec->prepare($query);
            ...
            if ($result['count'] > 0) {
                ...
                $query = "INSERT INTO tbtransportadores (codigo, nombre, resum, conduc, est) VALUES (:codigo, :nombre, :resumen, :conduc, :est)";
                $stmt = $dbconec->prepare($query);
                $stmt->bindParam(':codigo', $codigo);
                $stmt->bindParam(':nombre', $nombre);
                $stmt->bindParam(':resumen', $resumen);
                $stmt->bindParam(':conduc', $conduc);
                $stmt->bindParam(':est', $est);

                if ($stmt->execute()) {
                    // Si la inserción fue exitosa, devuelve un mensaje o los datos actualizados
                    $response = array('status' => 'success', 'message' => 'El transportador se ha guardado correctamente');
                } else {
                    // Si hubo un error en la inserción, devuelve un mensaje de error
                    $response = array('status' => 'error', 'message' => 'Error al guardar el transportador');
                }
    ...
    public static function modificar($datos)
    {
        ...
        try {
            ...
            $id = $datos['id'];
            // Consulta para verificar la existencia del código
            $query = "SELECT COUNT(*) as count, codigo FROM tbtransportadores WHERE id = :id";
            $stmt = $dbconec->prepare($query);
            ...
            if ($codigo != $codigo_bd) {
                $queryC = "SELECT COUNT(*) as count, codigo FROM tbtransportadores WHERE codigo = :codigo";
                ...
                if ($result['count'] > 0) {
                    ...
                    // Realiza la inserción en la base de datos (ajusta esto según tu configuración)
                    $query = "UPDATE tbtransportadores SET codigo=:codigo, nombre=:nombre, resum=:resumen, conduc=:conduc, est=:est WHERE id=:id";
                    $stmt = $dbconec->prepare($query);
                    $stmt->bindParam(':id', $id);
                    $stmt->bindParam(':codigo', $codigo);
                    $stmt->bindParam(':nombre', $nombre);
                    ...
                    if ($stmt->execute()) {
                        // Si la inserción fue exitosa, devuelve un mensaje o los datos actualizados
                        $response = array('status' => 'success', 'message' => 'El transportador se ha modificado exitosamente.');
                    } else {
                        // Si hubo un error en la inserción, devuelve un mensaje de error
                        $response = array('status' => 'error', 'message' => 'Error al modificar el transportador.');
                    }
    ...
    public static function eliminar($id)
    {
        ...
        try {
            $activo = 0;
            // Realiza la inserción en la base de datos (ajusta esto según tu configuración)
            $query = "UPDATE tbtransportadores SET activo=:activo WHERE id=:id";
            $stmt = $dbconec->prepare($query);
            $stmt->bindParam(':id', $id);
            ...
            if ($stmt->execute()) {
                // Si la inserción fue exitosa, devuelve un mensaje o los datos actualizados
                $response = array('status' => 'success', 'message' => 'transportador eliminado exitosamente.');
            } else {
                // Si hubo un error en la inserción, devuelve un mensaje de error
                $response = array('status' => 'error', 'message' => 'Error al eliminar el transportador.');
            }
